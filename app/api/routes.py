...
async def design(req: DesignRequest):
    return await design_session(req.user_text, req.user_id,
                                req.client_type or "mobile")
...
def status(sid: str):
    s = SESSIONS.get(sid, {})
    return {"state": s.get("state"), "t": s.get("t")}
...
async def rate(sid: str, req: RatingRequest):
    s = SESSIONS.get(sid)
    if s:
        await save_rating(s["blueprint"].user_id, sid,
                          s["blueprint"], req.rating, req.notes)
    return {"status": "saved"}
